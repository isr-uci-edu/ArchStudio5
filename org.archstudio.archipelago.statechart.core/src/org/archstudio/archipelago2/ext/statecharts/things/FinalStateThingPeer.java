...
public class FinalStateThingPeer<T extends FinalStateThing> extends AbstractThingPeer<T>
    implements IHasShadowPeer<T> {
  ...
  public boolean draw(Rectangle localBounds, IUIResources r) {
    ...
    Shape innerShape = new Ellipse2D.Double(lbb.x + delta, lbb.y + delta, lbb.width - 2 * delta + 1,
        lbb.height - 2 * delta + 1);
  ...
  public boolean isInThing(ICoordinate location) {
    ...
    return new Ellipse2D.Double(r.x, r.y, r.width, r.height)
        .contains(BNAUtils.toPoint2D(location.getWorldPoint()));
  }
