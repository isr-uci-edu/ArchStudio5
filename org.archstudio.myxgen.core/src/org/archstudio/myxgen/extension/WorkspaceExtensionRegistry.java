...
public class WorkspaceExtensionRegistry implements IExtensionRegistry {
	...
	public IConfigurationElement[] getConfigurationElementsFor(String namespace, String extensionPointName,
			String extensionId) {
		String extensionPointId = namespace + "." + extensionPointName;
		for (IPluginModelBase p : e.findExtensionPlugins(extensionPointId, true)) {
			if (p.getUnderlyingResource() != null) {
				for (IConfigurationElement c : PDEUtils.getExtensionPoint(extensionPointId,
						new IPluginModelBase[] { p }).getConfigurationElements()) {
					if (extensionId.equals(c.getAttribute("id"))) {
						return new IConfigurationElement[] { c };
					}
				}
			}
		}
