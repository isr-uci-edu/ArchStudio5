...
public class StructureMapper {
	...
	public synchronized static void removeOrphanedBricks(ArchipelagoServices AS, IBNAWorld bnaWorld, ObjRef structureRef) {
		...
		for (IThing t : bnaModel.getChildThings(brickRootThing)) {
			if (isBrickAssemblyRootThing(t)) {
				...
				if (xArchID != null) {
					BrickKind kind = t.get(BRICK_KIND_PROPERTY_NAME);
					if (kind == null) {
						continue;
					}
	...
	public synchronized static void updateBrick(ArchipelagoServices AS, IBNAWorld bnaWorld, ObjRef brickRef,
			BrickKind kind) {
		IBNAModel bnaModel = bnaWorld.getBNAModel();
		...
		AS.eventBus
				.fireArchipelagoEvent(new StructureEvents.BrickUpdatedEvent(bnaWorld, brickRef, kind, brickAssembly));
	}

	public synchronized static void updateSubarchitecture(ArchipelagoServices AS, IBNAWorld bnaWorld,
			BoxAssembly brickAssembly, ObjRef brickRef) {
		...
		if (subStructureRef != null) {
			...
			if (internalStructureRef != null) {
				...
				if (internalWorld != null) {
					...
					AS.eventBus.fireArchipelagoEvent(new StructureEvents.SubarchitectureUpdatedEvent(bnaWorld,
							brickAssembly, brickRef));
					return;
				}
			}
		}
		...
		AS.eventBus.fireArchipelagoEvent(new StructureEvents.SubarchitectureUpdatedEvent(bnaWorld, brickAssembly,
				brickRef));
	}

	public static void removeOrphanedInterfaceMappings(ArchipelagoServices AS, IBNAWorld bnaWorld,
			BoxAssembly brickAssembly, ObjRef brickRef, IBNAWorld innerWorld) {
		List<ObjRef> interfaceRefs = AS.xarch.getAll(brickRef, "interface");
		...
		if (subStructureRef != null) {
			...
			for (IThing t : bnaModel.getChildThings(brickAssembly.getMappingRootThing())) {
				if (t instanceof IHasAssemblyData
						&& ((IHasAssemblyData) t).getAssemblyKind().equals(MappingAssembly.ASSEMBLY_KIND)) {
					String xArchID = t.get(ArchipelagoUtils.XARCH_ID_PROPERTY_NAME);
					if (xArchID != null) {
						boolean found = false;
	...
	public static void updateInterfaceMapping(ArchipelagoServices AS, IBNAWorld bnaWorld, BoxAssembly brickAssembly,
			ObjRef brickRef, IBNAWorld innerWorld, ObjRef imRef) {
		List<ObjRef> interfaceRefs = AS.xarch.getAll(brickRef, "interface");
		...
		if (outerInterfaceRef != null && outerInterfaceXArchID != null) {
			...
			for (IThing iimAssemblyRootThing2 : iimAssemblyRootThings) {
				String iimAssemblyRootThingBrickXArchID = (String) iimAssemblyRootThing2.get("interfaceXArchID");
				if (iimAssemblyRootThingBrickXArchID != null
						&& iimAssemblyRootThingBrickXArchID.equals(outerInterfaceXArchID)) {
					iimAssemblyRootThing = iimAssemblyRootThing2;
				}
			}
			...
			if (outerInterfaceAssemblyRootThing != null && outerInterfaceAssemblyRootThing instanceof IHasAssemblyData) {
				EndpointAssembly outerInterfaceAssembly = EndpointAssembly.attach(bnaModel,
						(IHasAssemblyData) outerInterfaceAssemblyRootThing);
				if (outerInterfaceAssembly.getEndpointGlassThing() != null) {
					iimAssembly.getMappingGlassThing().setExternalEndpointStuckToID(
							outerInterfaceAssembly.getEndpointGlassThing().getID());
				}
			}
			...
			if (interfaceAssemblyRootThing != null && interfaceAssemblyRootThing instanceof IHasAssemblyData) {
				EndpointAssembly interfaceAssembly = EndpointAssembly.attach(innerModel,
						(IHasAssemblyData) interfaceAssemblyRootThing);
				if (interfaceAssembly.getEndpointGlassThing() != null) {
					iimAssembly.getMappingGlassThing().setInternalEndpointStuckToThingID(
							interfaceAssembly.getEndpointGlassThing().getID());
					iimAssembly.getMappingGlassThing().setEndpoint2(
							interfaceAssembly.getEndpointGlassThing().getAnchorPoint());
				}
			}
			AS.eventBus.fireArchipelagoEvent(new StructureEvents.InterfaceMappingUpdatedEvent(bnaWorld, brickAssembly,
					brickRef, innerWorld, imRef, iimAssembly));
		}
	}

	public synchronized static void removeOrphanedInterfaces(ArchipelagoServices AS, IBNAWorld bnaWorld,
			BoxAssembly brickAssembly, ObjRef brickRef) {
		IBNAModel bnaModel = bnaWorld.getBNAModel();
		...
		for (IThing t : bnaModel.getChildThings(brickAssembly.getEndpointRootThing())) {
			if (t instanceof IHasAssemblyData
					&& ((IHasAssemblyData) t).getAssemblyKind().equals(EndpointAssembly.ASSEMBLY_KIND)) {
				String xArchID = t.get(ArchipelagoUtils.XARCH_ID_PROPERTY_NAME);
				if (xArchID != null) {
					List<ObjRef> interfaceRefs = AS.xarch.getAll(brickRef, "interface");
	...
	public synchronized static void updateInterface(ArchipelagoServices AS, IBNAWorld bnaWorld,
			BoxAssembly brickAssembly, ObjRef interfaceRef) {
		IBNAModel bnaModel = bnaWorld.getBNAModel();
		...
		if (endpointAssemblyRootThing == null || !(endpointAssemblyRootThing instanceof IHasAssemblyData)) {
			endpointAssembly = EndpointAssembly
					.create(bnaModel, brickDescription, brickAssembly.getEndpointRootThing());
			endpointAssembly.getRootThing().setProperty(ArchipelagoUtils.XARCH_ID_PROPERTY_NAME, xArchID);

			endpointAssembly.getEndpointGlassThing().setAnchorPoint(
					new Point(brickAssembly.getBoxGlassThing().getBoundingBox().x, brickAssembly.getBoxGlassThing()
							.getBoundingBox().y));

			endpointAssembly.getBoxThing().setColor(new RGB(0xff, 0xff, 0xff));

			endpointAssembly.getEndpointGlassThing().setBoundingBoxRailMasterThingID(
					brickAssembly.getBoxGlassThing().getID());
		...
		AS.eventBus.fireArchipelagoEvent(new StructureEvents.InterfaceUpdatedEvent(bnaWorld, brickAssembly,
				interfaceRef, endpointAssembly));
	}
	...
	public synchronized static void removeOrphanedLinks(ArchipelagoServices AS, IBNAWorld bnaWorld, ObjRef structureRef) {
		...
		for (IThing t : bnaModel.getChildThings(linkRootThing)) {
			if (t instanceof IHasAssemblyData
					&& ((IHasAssemblyData) t).getAssemblyKind().equals(StickySplineAssembly.ASSEMBLY_KIND)) {
				String xArchID = t.get(ArchipelagoUtils.XARCH_ID_PROPERTY_NAME);
				if (xArchID != null) {
					List<ObjRef> linkRefs = AS.xarch.getAll(structureRef, "link");
					boolean found = false;
	...
	public synchronized static void updateLink(ArchipelagoServices AS, IBNAWorld bnaWorld, ObjRef linkRef) {
		...
		if (endpoint1TargetRef != null) {
			...
			if (endpoint1TargetID != null) {
				...
				EndpointAssembly endpointAssembly = EndpointAssembly.attach(bnaModel,
						(IHasAssemblyData) endpoint1TargetThing);
				if (endpoint1TargetThing != null) {
					linkAssembly.getSplineGlassThing().setEndpoint1StuckToThingID(
							endpointAssembly.getEndpointGlassThing().getID());
					endpoint1Stuck = true;
				}
			}
		}
		...
		if (endpoint2TargetRef != null) {
			...
			if (endpoint2TargetID != null) {
				...
				EndpointAssembly endpointAssembly = EndpointAssembly.attach(bnaModel,
						(IHasAssemblyData) endpoint2TargetThing);
				if (endpoint2TargetThing != null) {
					linkAssembly.getSplineGlassThing().setEndpoint2StuckToThingID(
							endpointAssembly.getEndpointGlassThing().getID());
					endpoint2Stuck = true;
				}
			}
	...
	public static boolean isComponentAssemblyRootThing(IThing t) {
		if (t instanceof AssemblyRootThing) {
			String assemblyType = t.get(ASSEMBLY_TYPE_PROPERTY_NAME);
