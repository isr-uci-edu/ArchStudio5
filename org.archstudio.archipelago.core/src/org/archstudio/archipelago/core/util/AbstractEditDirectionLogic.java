...
public abstract class AbstractEditDirectionLogic extends AbstractThingLogic implements IBNAMenuListener,
	...
	public void fillMenu(IBNAView view, IMenuManager m, int localX, int localY, IThing t, int worldX, int worldY) {
		List<IThing> selectedThings = BNAUtils.getSelectedThings(view.getBNAWorld().getBNAModel());
	...
	protected IAction[] getActions(IBNAView view, IThing t, int worldX, int worldY) {
		...
		Action editDirectionAction = new Action("Edit Direction...") {
			...
			public void run() {
				...
				lt.setEditing(true);
				openControls.add(lt);
				fview.getBNAWorld().getBNAModel().addThing(ft, lt);
			}
		};
