...
public class XArchADTImplTest extends AbstractXArchADTTest {
	...
	public void testXQuery() throws XPathException {
		...
		assertEquals(
				XArchADTProxy.unproxy(Lists.newArrayList(i1, i2)),
				xarch.resolveObjRefs(XArchADTProxy.unproxy(xadlType), "/topLevelElement/component[@id='abc']/interface"));
		assertEquals(
				XArchADTProxy.unproxy(Lists.newArrayList(i3, i4)),
				xarch.resolveObjRefs(XArchADTProxy.unproxy(xadlType), "/topLevelElement/component[@id='def']/interface"));
		assertEquals(
				Lists.newArrayList(Structure_3_0Package.eNS_URI),
