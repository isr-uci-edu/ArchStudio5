...
public class BNACanvas extends GLCanvas implements IBNAModelListener, PaintListener {
	...
	public void paintControl(PaintEvent evt) {
		...
		try {
			...
			Rectangle clipBounds = new Rectangle(0, 0, bounds.width, bounds.height);
			try {
			...
			for (IThing thingToRender : bnaModel.getAllThings()) {
				IThingPeer<?> peer = getBNAView().getThingPeer(thingToRender);
				if (clipBounds.intersects(peer.getLocalBounds(getBNAView(), getBNAView().getCoordinateMapper()))) {
					gl.glPushMatrix();
					gl.glPushAttrib(GL2.GL_TRANSFORM_BIT | GL2.GL_LINE_BIT | GL2.GL_CURRENT_BIT);
					try {
						peer.draw(getBNAView(), getBNAView().getCoordinateMapper(), gl, clipBounds, resources);
					}
					catch (Exception e) {
						e.printStackTrace();
					}
