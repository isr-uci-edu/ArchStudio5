...
import org.archstudio.bna.IBNAFocusListener;
...
public class WorldThingExternalEventsLogic extends AbstractThingLogic implements IBNAMouseListener, IBNAMouseMoveListener, IBNAMouseTrackListener,
	...
	protected void mouseEvt(IBNAView view, MouseEvent e, IThing t, MouseEventType type) {
		...
		if ((type == MouseEventType.MOUSE_UP) || (type == MouseEventType.MOUSE_DOWN) || (type == MouseEventType.MOUSE_CLICK) || (type == MouseEventType.MOUSE_DOUBLECLICK)) {
			List<IBNAMouseListener> logics = internalWorld.getThingLogicManager().getThingLogics(IBNAMouseListener.class);
			if (!logics.isEmpty()) {
			...
			List<IBNAMouseMoveListener> logics = internalWorld.getThingLogicManager().getThingLogics(IBNAMouseMoveListener.class);
			if (!logics.isEmpty()) {
				IThing internalThing = internalView.getThingAt(e.x, e.y);
				int worldX = internalView.getCoordinateMapper().localXtoWorldX(e.x);
				int worldY = internalView.getCoordinateMapper().localYtoWorldY(e.y);
			...
			List<IBNAMouseTrackListener> logics = internalWorld.getThingLogicManager().getThingLogics(IBNAMouseTrackListener.class);
