...
public class MaintainMappingEndpointsLogic extends AbstractThingLogic implements IInternalBNAModelListener, IThingSetListener, IBNASynchronousModelListener {

	protected ThingRefTrackingLogic trtl = null;
	protected TypedThingSetTrackingLogic<MappingGlassThing> ttstl = null;
	protected WorldThingInternalEventsLogic vtiel = null;

	public MaintainMappingEndpointsLogic(ThingRefTrackingLogic trtl, TypedThingSetTrackingLogic<MappingGlassThing> ttstl, WorldThingInternalEventsLogic vtiel) {
		this.trtl = trtl;
		this.ttstl = ttstl;
		this.vtiel = vtiel;
	}
				...
				if (possibleViewThing instanceof IHasWorld) {
					...
					if (internalWorld != null) {
						...
						if (im != null) {
							...
							if ((internalEndpointStuckToThing != null) && (internalEndpointStuckToThing instanceof IHasStickyBox)) {
								Rectangle sb = ((IHasStickyBox) internalEndpointStuckToThing).getStickyBox();
								if (sb != null) {
									if (!BNAUtils.isPointOnRectangle(internalPoint, sb)) {
										// The endpoint is stuck, but its endpoint is not on the sticky box.
										PointToRectangleDistanceData dd = BNAUtils.getPointToRectangleDistance(internalPoint, sb);
		...
		if (t instanceof MappingGlassThing) {
			...
			if (te != null) {
				...
				if (propertyName != null) {
					if (propertyName.equals(MappingGlassThing.EXTERNAL_ENDPOINT_STUCK_TO_THING_ID_PROPERTY_NAME)
					        || propertyName.equals(MappingGlassThing.WORLD_THING_ID)
					        || propertyName.equals(MappingGlassThing.INTERNAL_ENDPOINT_STUCK_TO_THING_ID_PROPERTY_NAME)) {
						checkEndpoints(st);
					}
