...
public class PopulateAscending extends ComplexTerm implements Executable {

	public PopulateAscending(String name, List<? extends Term> terms) {
		super(name, 3, terms);
	}
	...
	public Iterable<Map<VariableTerm, Term>> execute(ProofContext proofContext, UnificationEngine unificationEngine,
		...
		List<Term> compareList = listTerm.asList();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (i > 0) {
				conjunction.add(proofContext.create("@<", Lists.newArrayList(list.get(i - 1), list.get(i))));
			}
		}
