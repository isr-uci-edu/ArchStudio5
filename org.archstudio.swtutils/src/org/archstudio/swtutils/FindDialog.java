...
public class FindDialog<C> extends Dialog{
    ...
    RGB result = csd.open(new RGB(0,0,0));
	...
	*/
	...
	public void open(final C context, String initialValue){
		...
		final TreeViewer resultViewer = new TreeViewer(mainComposite, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
		...
		GridData resultViewerData = new GridData(GridData.HORIZONTAL_ALIGN_FILL | GridData.GRAB_HORIZONTAL | GridData.VERTICAL_ALIGN_FILL | GridData.GRAB_VERTICAL);
	...
	class ResultsTreeLabelProvider extends LabelProvider implements ILabelProvider{
		...
		public Image getImage(Object element){
			if(element instanceof IFindResult){
				Image img = resultImages.get((IFindResult)element);
				if(img == null){
		...
		public void dispose(){
			Image[] images = (Image[])resultImages.values().toArray(new Image[resultImages.size()]);
